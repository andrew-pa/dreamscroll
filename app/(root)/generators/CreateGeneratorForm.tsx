...
import { GENERATOR_TYPES, GeneratorType } from '@/lib/repositories/generatorRepository';
...
export default function CreateGeneratorForm() {
  ...
  const submit = () =>
    startTransition(async () => {
      if (!name.trim()) {
        toaster.create({ type: 'warning', description: 'Name is required' });
        return;
      }
      await createGenerator(name.trim(), type);
      toaster.create({ type: 'success', description: 'Generator created' });
      setName('');
      ...
      <Card.Body>
        <HStack flexWrap="wrap" gap={4}>
          ...
          <Field.Root required>
            ...
            <NativeSelect.Field
              value={type}
              onChange={(e) => setType(e.currentTarget.value as GeneratorType)}
            >
